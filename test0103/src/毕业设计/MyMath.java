...
public class MyMath {
    ...
    //求一组数据的标准差
    public static double getStd(double[] input){
        double sum = 0;
        for(int i=0;i<input.length;i++){
            sum += input[i];      //求出数组的总和
        }
        double average = sum/input.length;  //求出数组的平均数
        double total=0;
        for(int i=0;i<input.length;i++){
            total += (input[i]-average)*(input[i]-average);   //求出方差，如果要计算方差的话这一步就可以了
        }
        double standardDeviation = Math.sqrt(total/input.length);   //求出标准差
